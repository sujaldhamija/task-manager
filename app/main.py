...
def user_login(username, password):
    ...
    del e[
        "auth_token_expiry"
    ]  # was giving some error and it was unnecessary to return so removed it
    return e
...
def create_task():
    ...
    try:
        ...
        if (
            "title" not in post_data
            or "desc" not in post_data
            or "due_date" not in post_data
        ):
        ...
        mycursor.execute(
            query,
            (post_data["title"], post_data["desc"], post_data["due_date"], user_check),
        )
...
def update_task():
    ...
    try:
        if "auth" not in post_data:
            ...
            "title" not in post_data
            and "desc" not in post_data
            and "due_date" not in post_data
            and "status" not in post_data
        ):
...
def update_access_task():
    ...
    try:
        if "auth" not in post_data:
            ...
            "task_id" not in post_data
            or "type" not in post_data
            or "users" not in post_data
        ):
            raise Exception(
                '"Task id" or "type" or ""users" to assign or remove" is missing'
            )
